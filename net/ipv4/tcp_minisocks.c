			   ...
			   bool fastopen, bool *req_stolen)
{
	...
	// TCP_DEFER_ACCEPT选项: 三次握手完成后, 并不accept, 等到客户端数据来到再accept. 减少accept造成的系统调用,优化性能
	...
	//正常情况下own_req=true。 req_stolen = false
	*req_stolen = !own_req;

    // 从半连接中移除req(实际上只是半连接计数器--)。sk加入全连接(accept)队列，是真的存在一个FIFO。
